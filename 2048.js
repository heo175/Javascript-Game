...
function 랜덤생성() {
	...
	if(빈칸배열.length === 0){ // 빈칸이 없으면 게임 오버
		alert('게임 오버: '+점수표.textContent);
		테이블.innerHTML = '';
		데이터 = [];
		초기화();
		점수표.textContent = '0';
...
var 시작좌표; 
var 끝좌표;
// screenX : 모니터 기준 좌표, pageX : 페이지(스크롤 포함), clientX : 브라우저 화면 기준, offsetX : 이벤트 타겟 기준
// 마우스 누를 때
...
window.addEventListener('mouseup', function(이벤트) {
	...
	if(드래그중){
		...
		if(x차이 < 0 && Math.abs(x차이)/Math.abs(y차이) > 1){ // Math.abs() : 절댓값
			방향 = '왼쪽';
