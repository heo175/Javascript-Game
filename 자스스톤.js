...
function 덱에서필드로(데이터, 내턴){
	...
	var idx = 객체.덱data.indexOf(데이터); // 내 덱 데이터가 몇 번째인지 찾기
...
// 턴이 끝난 카드면 아무일도 일어나지 않음
	var 아군 = 내턴 ? 나 : 상대;
	var 적군 = 내턴 ? 상대 : 나;
	if(카드.classList.contains('card-turnover')){ // 턴이 끝난 카드면 아무일도 안 일어남
		return;
	}
	// 적군 카드고 아군 카드가 선택되어 있으면
	...
	if(적군카드 && 아군.선택카드){ 
		...
		아군.선택카드data = null;
		return;
	}else if(적군카드){ // 상대 카드면 바로 종료
		return;
	}
	if(데이터.field){ // 카드가 필드에 있으면
		// 영웅 부모와 필드카드의 부모가 다르기 때문에 document에서 모든 .card를 검색
...
function 카드돔연결(데이터, 돔, 영웅){
	var 카드 = document.querySelector('.card-hidden .card').cloneNode(true); // cloneNode : 기존 태그를 그대로 복사, true를 넣으면 내부까지 다 복사
